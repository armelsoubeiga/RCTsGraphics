#==========================================================#
## 14 mars 2019
## Created by armel soubeiga
## Shiny Application to  easyly the graphics of RCTs Compagny
##
...
shinyUI(tagList(shinyjs::useShinyjs(),
    includeCSS("css/rctsgraph.css"),
    div(id = "loader",
    h2("Loading..."),
    tags$style(type="text/css", ".recalculating {opacity: 1.0;}" )),
    
    navbarPage(title = "RCTs-Graphics", id = "navbar", fluid = TRUE, theme = shinytheme("flatly"),
    tabPanel(title = "Data", icon = icon("table"),
          ...
          sidebarPanel(
            fluidRow(
              column(6,
                     radioButtons(inputId = "typefile", label =  '',inline = TRUE,
                                  choices = c(csv = "csv", 
                                              sas = "sas", 
                                              excel = 'xml'),
                                  selected = "csv"))
    ...
    tabsetPanel(
      tabPanel("MAIN",
               fluidRow(
                 ...
                 uiOutput(outputId = "loess_out_sc"),
                  ...
                  checkboxInput(inputId = "customize_l_sc",label = "Customize:",
                   ...
                   colourInput(inputId = "loess_col1_sc", label = "Colour:",
                    ...
                    conditionalPanel( 
                    condition = "input.errbar_sc !== 'none'",
                    checkboxInput(inputId = "customize_err_sc",label = "Customize:",
                                        value = FALSE)),
                 

                    conditionalPanel(
                    condition = "input.customize_err_sc && input.errbar_sc !== 'none'",
                 ...
                 # Grid
                 conditionalPanel(
                 condition = "input.faceting_sc === 'Grid'",
