...
# ============================= DATA SECTION : ================================    
  ...
  uploadedData <- reactive({
    ...
    if(is.null(upFile)){
      ...
      if(input$typefile=="csv"){
        read.csv(upFile$datapath, header = input$header, sep = input$sep,
                 quote = input$quote, dec = input$dec)
      } else{
        if(input$typefile=="sas"){
          read_sas(upFile$datapath)
        } else {
          if(input$typefile=="xml"){
            read.xls(upFile$datapath, sheet = 1, header = TRUE)
          }
        }
      }
    }
  ...
  output$table <- DT::renderDataTable(
    ...
    extensions = "Buttons",
    options = list(
      lengthMenu = list(c(10,25, 50, 100,  -1), c("25", "50", "100", "All")),
